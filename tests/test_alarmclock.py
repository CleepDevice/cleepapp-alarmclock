...
class TestsAlarmclock(unittest.TestCase):
    ...
    def test_add_alarm_invalid_parameters(self):
        ...
        with self.assertRaises(InvalidParameter) as cm:
            self.module.add_alarm({'hour':1, 'minutes': 1}, 10, days, False, 50, False, False)
        ...
        with self.assertRaises(InvalidParameter) as cm:
            self.module.add_alarm(time_, 10, {'mon': True, 'other': True}, False, 50, False, False)
        self.assertEqual(str(cm.exception), 'Parameter "days" is invalid or no day is selected')
        with self.assertRaises(InvalidParameter) as cm:
            self.module.add_alarm(time_, 10, {'mon': False, 'tue': False, 'wed': False, 'thu': False, 'fri': False, 'sat': False, 'sun': False}, False, 50, False, False)
        self.assertEqual(str(cm.exception), 'Parameter "days" is invalid or no day is selected')
    ...
    def test_remove_alarm(self):
        ...
        alarm = { 'time': {'hour': 1, 'minute': 1}, 'timeout': 10, 'enabled': True, 'volume': 50, "repeat": False, "shuffle": False }
    ...
    def test_toggle_alarm_enable(self):
        self.init()
        alarm = { 'time': {'hour': 1, 'minute': 1}, 'timeout': 10, 'enabled': False, 'volume': 50, "repeat": False, "shuffle": False }
        self.module._get_device = Mock(return_value=alarm)
        self.module._update_device = Mock(return_value=True)
...
class TestsAlarmScheduledToAlarmFormatter(unittest.TestCase):
    ...
    def setUp(self):
        logging.basicConfig(level=logging.FATAL, format=u'%(asctime)s %(name)s:%(lineno)d %(levelname)s : %(message)s')
        events_broker = Mock()
        self.formatter = AlarmScheduledToAlarmFormatter({'events_broker': events_broker})
...
class TestsAlarmUnscheduledToAlarmFormatter(unittest.TestCase):
    ...
    def setUp(self):
        logging.basicConfig(level=logging.FATAL, format=u'%(asctime)s %(name)s:%(lineno)d %(levelname)s : %(message)s')
        events_broker = Mock()
        self.formatter = AlarmUnscheduledToAlarmFormatter({'events_broker': events_broker})
...
class TestsAlarmTriggeredToAlarmFormatter(unittest.TestCase):
    ...
    def setUp(self):
        logging.basicConfig(level=logging.FATAL, format=u'%(asctime)s %(name)s:%(lineno)d %(levelname)s : %(message)s')
        events_broker = Mock()
        self.formatter = AlarmTriggeredToAlarmFormatter({'events_broker': events_broker})
